...
export function EventCard({ event, onBuy, isProcessing }: EventProps) {
  return (
    <div className="bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-card hover:shadow-xl transition-all group flex flex-col h-full">
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <button 
          ...
          className="w-full py-3.5 rounded-xl font-bold bg-white border-2 border-brand-100 text-brand-600 hover:bg-brand-600 hover:text-white hover:border-brand-600 transition-all shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {isProcessing ? "Processing..." : (
            <>
              <Tag size={18} /> Buy Ticket
            </>
          )}
        </button>
      </div>
